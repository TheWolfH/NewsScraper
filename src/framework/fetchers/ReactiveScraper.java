...
public abstract class ReactiveScraper extends Scraper {
	 ...
	 * {@link framework.articles.Article} objects representing articles that
	 * contain one or more of the strings in {@code keywords} and that were
	 * published on or after {@code fromDate} and on or before {@code toDate},
	 * mapped to their URL. This method uses
	 * {@link #getFirstSearchURL(String, Date, Date, int)} and GETNEXTSEARCHURL
	 * to iterate over the pagination of the search function of the scraped news
	 * site, calls the returned URL and retrieves all search framework.results
	 * using the selector provided by {@link #getSearchResultsSelector()}. These
	 * framework.results are iterated over, extracting the URL and the title
	 * using the {@link #getUrlFromSearchResult(Element)} and
	 * {@link #getTitleFromSearchResult(Element)} methods and creating objects
	 * of a subclass of {@link framework.articles.Article} using the
